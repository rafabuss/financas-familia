...
export default function App() {
  ...
  const [seriesEditModal, setSeriesEditModal] = useState({ isOpen: false, transaction: null });

  // Recálculo do Saldo das Contas
  ...
  }, [transactions, accountBalances, cardInvoices]);
  ...
  // Iniciar Conversão de Cenário para Real
  ...
  const handleSimulatePdfImport = () => {
    const sampleItems = [
      { id: 'imp-1', description: 'Netflix & Spotify', amountCents: 7990, date: '15/09/2026', categoryId: 'cat-7' },
      { id: 'imp-2', description: 'Posto Ipiranga Combustível', amountCents: 21050, date: '18/09/2026', categoryId: 'cat-8' },
      { id: 'imp-3', description: 'Farmácia Panvel Medicamentos', amountCents: 14520, date: '20/09/2026', categoryId: 'cat-6' },
      { id: 'imp-4', description: 'Almoço Restaurante SC-401', amountCents: 18500, date: '22/09/2026', categoryId: 'cat-7' },
    ];
  ...
  const handleConfirmImport = () => {
    ...
    setTransactions([...transactions, ...newTxs]);
    alert(`${toImport.length} lançamentos gravados com sucesso!`);
  ...
  const exportData = (format) => {
    ...
    if (format === 'json') {
      ...
      const csvRows = [
        ...
        ...transactions.map(
          (t) => `"${t.id}","${t.description}",${t.amountCents},"${t.type}","${t.status}","${t.date}","${t.categoryId}","${t.accountId || ''}","${t.cardId || ''}"`
        ),
      ];
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8">
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-6">
            ...
            {/* 2 Quadros de Detalhe: Próximos Vencimentos e Cartões */}
                          ...
                          >
                            <span
                              className={`w-1.5 h-1.5 rounded-full mr-1.5 ${
                                tx.status === 'REALIZADO' ? 'bg-emerald-500' : 'bg-blue-500'
                              }`}
                            />
                    ...
                    return (
                      <div key={card.id} className="space-y-2">
                        ...
                        <div className="flex justify-between text-xs text-slate-600 pt-0.5">
                          <span>
                            Comprometido: {formatMoney(info.invoiceTotalCents)} ({pct}%)
                          </span>
                          <span className="font-semibold text-slate-900">
                            Disponível: {formatMoney(info.availableCents)}
                          </span>
                        </div>
                      </div>
                    );
        ...
        {/* ===================== ABA: CENÁRIOS & SIMULAÇÕES (TELA 2 EXATA) ===================== */}
        {activeTab === 'scenarios' && (
          <div className="space-y-6">
              ...
              {scenarios.map((scen) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Rodapé: Toggle Switch e Botão Converter em Real */}
                    <div className="mt-5 pt-2 flex items-center justify-between">
                      {/* Interruptor Pill Switch */}
                      <button
                        type="button"
                      ...
                      {/* Botão de Conversão em Lançamento Real */}
                      <button
                        type="button"
                  ...
                  <div key={card.id} className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center pb-4 border-b border-slate-100 gap-2">
                      <div>
                        ...
                        <p className="text-xs text-slate-400 mt-0.5">
                          Fechamento todo dia {card.closingDay} • Vencimento todo dia {card.dueDay}
                        </p>
                      </div>
                    ...
                    <div className="mt-4">
                      <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">
                        LANÇAMENTOS E PARCELAS VINCULADAS A ESTE CARTÃO:
                      </h4>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
              <div>
                ...
                <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
                  Previsão acumulada considerando receitas, parcelas futuras e cenários ativos.
                </p>
              </div>
