...
export function FinanceProvider({ children }) {
  ...
  const maskTransactionForViewer = useCallback((tx) => {
    ...
    if (!isPersonalPrivate) {
      return { ...tx, isPersonalPrivate: false, isMasked: false };
    }
    ...
    const isOwner = (tx.ownerId && tx.ownerId === activeMemberKey) || 
                    (currentUser?.id && tx.ownerId === currentUser.id);

    if (isOwner) {
      return {
        ...tx,
        isPersonalPrivate: true,
        isMasked: false,
      };
    }
  ...
  const handleDeleteTransaction = (tx) => {
    if (tx?.isMasked) {
      alert(`Este lançamento pessoal pertence a ${tx.maskedOwnerName || 'outro membro'} e está protegido por privacidade. Apenas o titular pode excluí-lo.`);
      return;
    }
    setDeleteModalState({
      isOpen: true,
  ...
  const toggleStatusPaid = (tx) => {
    if (tx?.isMasked) {
      alert(`Este lançamento pessoal pertence a ${tx.maskedOwnerName || 'outro membro'} e está protegido por privacidade.`);
      return;
    }
