-- ==============================================================================
-- FINANÇAS DA FAMÍLIA — MIGRATION FASE 4: MULTI-TENANCY SEGURO & RLS RESTRITO
-- ==============================================================================
-- Execute este script no "SQL Editor" do painel do Supabase.
-- Ele é estritamente idempotente (pode ser executado múltiplas vezes com segurança).
-- 
-- Regras de Negócio e Segurança:
-- 1. Usuários existentes são preservados na "Família Principal".
-- 2. Novos usuários sem indicação de família criam automaticamente sua própria nova família (como OWNER).
-- 3. Novos usuários que indicam uma família (via código de convite) entram com status 'pending'
--    e NÃO possuem acesso aos dados até aprovação explícita do OWNER/ADMIN.
-- 4. RLS estrito: dados de uma família são 100% isolados de outras famílias.
-- ==============================================================================

-- 1. Criação da Tabela de Famílias (Households)
...
-- Garante a coluna invite_code caso a tabela já existisse
...
-- 2. Criação da Tabela de Membros da Família (Household Members)
...
-- Garante as colunas em instalações prévias
...
-- Índices para buscas e validações de membresia
...
-- 3. Adicionar household_id em todas as tabelas da aplicação
...
-- 4. Adicionar coluna visibility em transactions (Privacidade / Olho Amigo)
...
-- 5. Migração Segura dos Dados Existentes: Família Principal
-- Cria a Família Principal padrão para acolher os dados já cadastrados
...
-- Associa todos os usuários que já existem em profiles como membros 'active' da Família Principal
...
-- Vincula todos os registros pré-existentes sem household_id à Família Principal
...
-- Migra lançamentos antigos marcados como PERSONAL para visibility PERSONAL_PRIVATE
UPDATE public.transactions 
SET visibility = 'PERSONAL_PRIVATE' 
WHERE scope = 'PERSONAL' AND visibility = 'FAMILY';

-- 6. Índices de performance para multi-tenancy
...
-- 7. Função auxiliar para obter os IDs de famílias ativas do usuário logado (usada nas políticas RLS)
...
-- 8. Função / Trigger para Novos Usuários do Supabase Auth
-- REGRA FUNDAMENTAL:
-- Se o novo usuário informar um invite_code válido via metadados -> Entra com status 'pending' (aguardando aprovação).
-- Se NÃO informar código -> Cria automaticamente uma nova família exclusiva para ele e se torna 'owner' com status 'active'.
...
BEGIN
  ...
  -- Atualiza ou insere o perfil do usuário
  ...
  -- Verifica se foi informado um código de convite para ingressar em família existente
  IF v_invite_code IS NOT NULL THEN
    ...
    IF v_target_household_id IS NOT NULL THEN
      -- Ingressa como pendente (NÃO tem acesso aos dados até aprovação do admin da família)
      INSERT INTO public.household_members (household_id, user_id, role, status, display_name, member_key)
  ...
  -- Se não informou código ou o código não foi encontrado, cria uma NOVA família exclusiva
  INSERT INTO public.households (name)
  VALUES ('Família de ' || v_user_name)
  RETURNING id INTO v_new_household_id;

  -- Associa o criador como 'owner' com status 'active' da nova família
  INSERT INTO public.household_members (household_id, user_id, role, status, display_name, member_key)
  VALUES (v_new_household_id, NEW.id, 'owner', 'active', v_user_name, 'user-1')
  ON CONFLICT (household_id, user_id) DO NOTHING;

  -- Semeia categorias padrão para esta nova família
...
-- Reassocia a trigger ao evento de cadastro de novos usuários
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
CREATE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();

-- 9. Procedimentos para Gerenciamento de Membros (Aprovação / Rejeição)
-- Permite que administradores aprovem ou rejeitem membros pendentes
...
BEGIN
  -- Identifica a família do membro
  SELECT household_id INTO v_household_id FROM public.household_members WHERE id = p_member_id;
  IF v_household_id IS NULL THEN
    RAISE EXCEPTION 'Membro não encontrado.';
  END IF;

  -- Verifica se quem está aprovando é owner ou admin ATIVO dessa mesma família
  SELECT EXISTS (
...
-- 10. Políticas de Row Level Security (RLS) Estritas
...
-- Households: apenas membros ativos podem visualizar sua família
...
-- Household Members:
-- Visualização: membros ativos veem membros da sua família; solicitantes pendentes veem apenas seu próprio registro
...
-- Gerenciamento de membros: somente owner/admin ativo
...
-- Profiles: Leitura pública para autenticados da mesma família ou do próprio perfil
